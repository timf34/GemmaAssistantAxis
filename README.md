...
Companion track to [SynthenticPersonaPretrainingSprint](https://github.com/timf34/SynthenticPersonaPretrainingSprint), which runs the same pipeline on 3B synthetic-persona-pretrained models. Both tracks use the identical 90-persona subset (`roles_90.json`) and identical scale settings so per-model metrics are directly comparable; final cross-track comparison is assembled from the two tracks' `COMPARISON.md` files.
...
- `roles_90.json` — canonical 90-persona subset (30 helpers / 30 humans / 30 non-human), shared across tracks
